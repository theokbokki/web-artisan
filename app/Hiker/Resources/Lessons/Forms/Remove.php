...
class Remove extends Form
{
    ...
    {
        return 'Supprimer "'.$this->trip->resource()->model()->title.'"';
    }
    ...
    public function steps(Baggage $bag)
    {
        return [
            Step::make($this->subject(), 'step_one')
                ->fields([
                    DisplayTextfield::make('Êtes-vous sûr de vouloir supprimer **'.$this->trip->resource()->model()->title.'** ? Cette action est irréversible !'),
                ])
